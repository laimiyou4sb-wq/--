...
# Backend (run from backend/) — skeleton only, not yet functional
cd backend
...
- **Seed data**: `initSeedData()` runs on mount, checks `db.inspirations.count()`, bulk-inserts 22 sample inspirations + 2 boards + 3 diary entries if empty.
